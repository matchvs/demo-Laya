...
module mvs {
	...
	export class MsResponse extends Sprite{
		...
		private createRoomResponse(rsp:MsCreateRoomRsp){
			...
			this.event(MsEvent.EVENT_CREATEROOM_RSP, new MsEventData(data));
		}
		...
		private sendEventResponse(rsp:MsSendEventRsp){
			...
			this.event(MsEvent.EVENT_SENDEVENT_RSP, new MsEventData(data));
		}
		...
		private sendEventNotify(eventInfo:MsSendEventNotify){
			...
			this.event(MsEvent.EVENT_SENDEVENT_NTFY, new MsEventData(data));
		}
		...
		private gameServerNotify(eventInfo:MsGameServerNotifyInfo){
			...
			this.event(MsEvent.EVENT_GAMESERVER_NTFY, new MsEventData(data));
		}
		...
		private joinOverResponse(rsp:MsJoinOverRsp){
			...
			this.event(MsEvent.EVENT_JOINOVER_RSP, new MsEventData(data));
		}
		...
		private joinOverNotify(Info:MsJoinOverNotifyInfo){
			...
			this.event(MsEvent.EVENT_JOINOVER_NTFY, new MsEventData(data));
		}
		...
		private leaveRoomResponse(rsp:MsLeaveRoomRsp){
			...
			this.event(MsEvent.EVENT_LEAVEROOM_RSP, new MsEventData(data));
		}
		...
		private leaveRoomNotify(leaveRoomInfo:MsLeaveRoomNotify){
			...
			this.event(MsEvent.EVENT_LEAVEROOM_NTFY, new MsEventData(data));
		}
		...
		private networkStateNotify(netnotify:MsNetworkStateNotify){
			...
			this.event(MsEvent.EVENT_NETWORKSTATE_NTFY, new MsEventData(data));
		}
		...
		private frameUpdate(fd:MsFrameData){
			...
			this.event(MsEvent.EVENT_FRAMEUPDATE, new MsEventData(data));
		}
		...
		private errorResponse(errCode:number, errMsg:string){
			...
			this.event(MsEvent.EVENT_ERROR_RSP, new MsEventData(data));
		}
		...
		private logOutResponse(status:number){
			...
			this.event(MsEvent.EVENT_LOGOUT_RSP, new MsEventData(data));
		}
		...
		private kickPlayerNotify(knotify:MsKickPlayerNotify){
			...
			this.event(MsEvent.EVENT_KICKPLAYER_NTFY, new MsEventData(data));
		}
		...
		private setRoomPropertyNotify(notify:MsRoomPropertyNotifyInfo){
			...
			this.event(MsEvent.EVENT_SETROOMPROPERTY_NTFY, new MsEventData(data));
		}
		...
		private setRoomPropertyResponse(rsp:MsSetRoomPropertyRspInfo){
			...
			this.event(MsEvent.EVENT_SETROOMPROPERTY_RSP, new MsEventData(data));
		}
		/**
		 * 获取房间详细信息回调
		 */
		private getRoomDetailResponse(rsp:MsGetRoomDetailRsp){
			...
			this.event(MsEvent.EVENT_GETROOMDETAIL_RSP, new MsEventData(data));
		}
		...
		private getRoomListExResponse(rsp:MsGetRoomListExRsp){
			...
			this.event( MsEvent.EVENT_GETROOMLIST_EX_RSP,new MsEventData(data));
		}
		...
		private reconnectResponse(status:number, roomUserInfoList:Array<MsRoomUserInfo>, roomInfo:MsRoomInfo){
			...
			this.event( MsEvent.EVENT_RECONNECT_RSP,new MsEventData(data));
		}
